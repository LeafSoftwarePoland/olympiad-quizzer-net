...
public sealed class QuizSessionValidatorTests
{
    ...
    {
        bool result = QuizSessionValidator.IsValid(null, Now);
    ...
    public void IsValid_WithUnknownSchemaVersion_ReturnsFalse()
    {
        ...
        bool result = QuizSessionValidator.IsValid(state, Now);
    ...
    {
        QuizSessionState state = ValidState(1);
        state.Questions = new List<Question>();
        state.Answers   = new List<SubmittedAnswer>();
    ...
    public void IsValid_WithMoreQuestionsThanMaxLimit_ReturnsFalse()
    {
        ...
        List<Question> questions = Enumerable.Range(1, count)
            .Select(i => QuestionBuilder.AQuestion().WithId(i).Build())
            .ToList();
        List<SubmittedAnswer> answers = Enumerable.Range(0, count)
            .Select(_ => SubmittedAnswer.Empty)
            .ToList();
    ...
    {
        QuizSessionState state = ValidState(1);
        state.StartedAtUtc = Now.AddMinutes(10);
    ...
    public void IsValid_WithTamperedStartTimestampMovedBackwards_ReturnsTrueAndTimerReadsExpired()
    {
        ...
        state.StartedAtUtc     = Now.AddMinutes(-120);

        bool isValid    = QuizSessionValidator.IsValid(state, Now);
        TimeSpan timer  = ExamTimer.Remaining(state, Now);
    ...
    {
        List<Question> questions = Enumerable.Range(1, questionCount)
            .Select(i => QuestionBuilder.AQuestion().WithId(i).Build())
            .ToList();
        List<SubmittedAnswer> answers = Enumerable.Range(0, questionCount)
            .Select(_ => SubmittedAnswer.Empty)
            .ToList();
