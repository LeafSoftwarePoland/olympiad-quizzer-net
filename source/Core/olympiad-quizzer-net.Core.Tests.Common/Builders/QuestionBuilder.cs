...
namespace OlympiadQuizzer.Core.Tests.Common.Builders;

public sealed class QuestionBuilder
{
    private int _id = 1;
    private QuestionType _type = QuestionType.Single;
    private List<ContentBlock> _content = [new ContentBlock { Type = ContentBlockType.Text, Text = "Sample question?" }];
    private List<string> _category = ["general"];
    private string _olympiad = "OIJ";
    private string _stage = "E1";
    private List<string> _options = ["Option A", "Option B"];
    private List<string> _matchOptions = null;
    private List<string> _correctAnswer = ["Option A"];
    ...
    public static QuestionBuilder AFullyPopulatedQuestion()
    {
        ...
        builder._contentCpp       = [new ContentBlock { Type = ContentBlockType.Code, Text = "int main() {}" }];
        builder._matchOptions     = ["Match A", "Match B"];
        builder._explanation      = [new ContentBlock { Type = ContentBlockType.Text, Text = "Because..." }];
