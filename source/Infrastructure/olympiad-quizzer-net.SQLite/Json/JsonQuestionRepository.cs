...
public sealed class JsonQuestionRepository : IQuestionRepository
{
        ...
        ILogger<JsonQuestionRepository> logger)
    {
        ...
        _knownCategories = DistinctValues(_bank.Questions.SelectMany(q => q.Category ?? new List<string>()));
        _knownAlgorithms = DistinctValues(_bank.Questions.SelectMany(q => q.Algorithms ?? new List<string>()));
    ...
    public Task<IReadOnlyList<Question>> GetAsync(QuestionQuery query, CancellationToken cancellationToken)
    {
        ...
        List<Question> matched = candidates.ToList();
    ...
    public Task<FilterOptions> GetFilterOptionsAsync(CancellationToken cancellationToken)
    {
        ...
        FilterOptions options = new FilterOptions
        {
            ...
            Categories = CountBy(all.SelectMany(q => q.Category ?? new List<string>())),
            Algorithms = CountBy(all.SelectMany(q => q.Algorithms ?? new List<string>())),
    ...
    {
        HashSet<string> set = new HashSet<string>(TagComparer);
        if (values == null)
        {
            return set;
        }
    ...
    private static List<FilterOption> CountBy(IEnumerable<string> values)
    {
        return values
            ...
            .GroupBy(value => value, TagComparer)
            .Select(group => new FilterOption { Value = group.Key, Count = group.Count() })
            .OrderBy(option => option.Value, TagComparer)
            .ToList();
    }
