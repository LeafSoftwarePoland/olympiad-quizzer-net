...
## Amendment — 2026-08-12 — offline mode deferred further; server-side filtering dependency

**Adds:** Context for why PWA/offline remains deferred.

Server-side filtering (v1.0 decision) means offline mode now requires duplicate client-side filtering logic — a significant complexity increase. Offline mode deferred until server-side filtering is stable and a caching strategy for the full question bank is designed. Current quiz session caching (`localStorage`, up to 30 questions) is not a PWA offline solution — it only covers the active session.
