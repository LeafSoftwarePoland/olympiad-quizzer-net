...
## Amendment — 2026-08-12 — Polish snake_case exception for tag vocabulary

**Adds:** Tag identifier exception.

Tag identifiers (`category[]`, `algorithms[]` values) use Polish words, Latin characters only. Diacritics dropped: ą→a, ę→e, ó→o, ś→s, ł→l, ź/ż→z, ć→c, ń→n. Example: `złożoność` → `zlozonosc`.

Exception justified: tags are human-facing Polish domain concepts shown in the UI. English equivalents less recognizable to Polish educators. Tags are NOT code identifiers.

Full vocabulary in `docs/tags.md`.
